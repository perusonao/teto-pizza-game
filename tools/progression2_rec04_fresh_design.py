...
# Production Visual P1 (separate session; recorded as a dependency only, never modified here)
VISUAL_P1 = {"branch": "claude/w1-ingredient-visual-preview-mt4uxw", "status": "FINAL_PASS",
             "implementationSha": "39ce35cc80b17574ed85e60c71f989acb1b41237",
             "humanPassRecordSha": "802b023d0753eebd9bd4222d21f5fa33424df5d6"}
OWNER_DECISION_SOURCE = "Repo owner (perusonao) in the REC-04 session, 2026-09-25: REC-04 = APPROVED"
# The approved configuration.  validate() requires the simulated recommended candidate to match it.
APPROVED = {"gateKind": "DEX", "unlockFeeFactor": 0.0, "firstPackPaid": True, "trialPizzas": 0,
            "grantPizzas": 0, "packPizzas": 10, "priceSchedule": [60, 80, 100, 120],
            "refillPrices": [30, 40, 50, 60]}
...
def w1_matrix(result, recipes):
    ...
    for i in W1_NEW_INGREDIENTS:
        ...
        rows.append({
            ...
            "status": "DERIVED_FROM_CONFIRMED_RULES",
        })
    return rows
...
def compute_readiness(results, stress, rec, recipes, inputs):
    a = results[RECOMMENDED]
    sims = a["simulations"]
    matrix = w1_matrix(rec, recipes)
    reach = w1_reachability(rec, recipes)
    checks = {
        "w1Deadlock0": all(v["outcome"] == "COMPLETE" for v in sims.values()),
        "w1AllProfilesDiscover25of25": all(v["discovered"] == v["recipes"] == 25 for v in sims.values()),
        "unreachableRecipe0": not a["static"]["unreachableRecipes"],
        "circularPrerequisite0": not a["static"]["circularPrerequisites"],
        "uselessUnlock0": not a["static"]["uselessUnlockSteps"],
        "affordability": not any(f.startswith(("CURRENCY_STARVATION", "UNLOCKED_BUT_UNUSABLE", "STOCK_STARVATION"))
                                 for f in a["findings"]),
        "recommendedFindings0": not a["findings"],
        "negativeControls6of6": all(negative_controls(inputs).values()) and len(negative_controls(inputs)) == 6,
        "ingredientMatrix7of7": sorted(r["ingredient"] for r in matrix) == sorted(W1_NEW_INGREDIENTS),
        "w1RecipeReachability10of10": len(reach) == 10 and all(
            r["discoveredAtBake"]["low"] is not None and r["discoveredAtBake"]["standard"] is not None for r in reach),
        "scale101AllProfilesComplete": all(v["outcome"] == "COMPLETE"
                                           for v in stress["results"][RECOMMENDED]["simulations"].values()),
        "onboardingMargheritaOnly": a["static"]["onlyMargheritaAtStart"],
    }
    return {"checks": checks, "allPass": all(checks.values()),
            "note": "determinism and authority pins are verified by --check (double build + git object pins)"}
...
STATUS_LEGEND = {
    ...
    "CONFIRMED_OWNER_DECISION": "Approved by the owner as REC-04 OD-REC04-1..3 (2026-09-25).",
    "DERIVED_FROM_CONFIRMED_RULES": "Generated by the confirmed rules from current content; regenerated per wave (not hand-authored).",
...
def build_json(inputs):
    ...
    ladder = [{"step": s["step"], "kind": "MATERIAL", "items": s["items"], "keyRecipe": s["keyRecipe"],
               "newlyReachable": s["newlyReachable"], "recipesBefore": s["recipesBefore"],
               "dexRequired": s["dexRequired"], "gate": s["gate"], "tier": s["tier"],
               "packPrice": rec["items"][s["items"][0]]["packPrice"],
               "refillPrice": rec["items"][s["items"][0]]["refillPrice"],
               "status": "DERIVED_FROM_CONFIRMED_RULES"}
              for s in rec["steps"]]
    readiness = compute_readiness(results, stress, rec, recipes, inputs)
    doc = {
        "schemaVersion": 1,
        "kind": "REC-04 Fresh Design companion",
        "generatedBy": "tools/progression2_rec04_fresh_design.py",
        "verdict": "RESOLVED" if readiness["allPass"] else "NOT_READY",
        "readiness": readiness,
        "ownerDecisionSource": OWNER_DECISION_SOURCE,
        ...
        "recommended": {
            ...
            "rules": {
                "unlockModel": {"value": "Discovery Ladder: an ordered progression step s unlocks when Dex discovered count >= s (one step per new discovery)",
                                "decision": "OD-REC04-1", "status": "CONFIRMED_OWNER_DECISION"},
                "stepAuthority": {"value": "ordered progression steps (kind per step); W1 uses kind MATERIAL only; other kinds (e.g. capability/content) may be added later",
                                  "decision": "OD-REC04-1", "status": "CONFIRMED_OWNER_DECISION"},
                "starsNotUnlockCurrency": {"value": "stars are not a material unlock condition; stars remain for skill/quality, achievements, later capability/content gates, Lunch Rush",
                                           "decision": "OD-REC04-1", "status": "CONFIRMED_OWNER_DECISION"},
                "ladderGeneration": {"value": "key-recipe rule: each step = smallest ingredient set completing >=1 new recipe (tie: most new recipes, most reuse, id); concrete order regenerated per wave",
                                     "status": "DERIVED_FROM_CONFIRMED_RULES"},
                "unlockFee": {"value": 0, "decision": "OD-REC04-2", "status": "CONFIRMED_OWNER_DECISION"},
                "firstStockOnUnlock": {"value": 0, "noFreeStarterStock": True,
                                       "ux": "NEW MATERIAL notice -> Shop -> first pack purchase -> Free Cooking; 'unlocked but no idea where to buy' is forbidden",
                                       "decision": "OD-REC04-2", "status": "CONFIRMED_OWNER_DECISION"},
                "onboardingStarters": {"value": "tomato-sauce / mozzarella / basil keep the existing onboarding starter authority (unlimited, not overridden by REC-04)",
                                       "status": "CONFIRMED"},
                "packModel": {"value": "1 Shop purchase = 10 pizza plays of stock; pack stock = 10 x k (k = max minCount of the ingredient)",
                              "packPizzas": 10, "decision": "OD-REC04-3", "status": "CONFIRMED_OWNER_DECISION"},
                "priceTiers": {"value": PRICE_TIERS, "decision": "OD-REC04-3", "status": "CONFIRMED_OWNER_DECISION"},
                "refillPrices": {"value": {t["tier"]: math.ceil(t["packPrice"] * REFILL_PRICE_FACTOR) for t in PRICE_TIERS},
                                 "rule": "50% of the tier price", "decision": "OD-REC04-3", "status": "CONFIRMED_OWNER_DECISION"},
                "tierBands": {"value": "T1 steps 1-5, T2 6-14, T3 15-29, T4 30+", "status": "DERIVED_FROM_CONFIRMED_RULES"},
                "units": {"storageUnit": "unchanged from main: scatter = pieces, spread = uses (saved inventory key/value/schemaVersion 2 unchanged)",
                          "displayUnit": "pizza plays, e.g. 'ピザ10枚分（30個）'",
                          "packInStorageUnits": "10 x k (pieces for scatter; 10 uses for spread since k = 1)",
                          "migrationAuthority": "piece/portion migration NOT decided on main (#214 D-2 M4 is RECORDED_IN_OPEN_PR, #205 uses-model unmerged); REC-04 changes no save schema",
                          "status": "CONFIRMED_OWNER_DECISION"},
                "hamKChangeForW1": {"value": "ham k 1 -> 3 when W1 ships (pizza-portuguesa ham x3): explicit balance change inside the W1 data slice",
                                    "status": "PROPOSED"},
                "entitlement": {"value": "an unlocked step never re-locks when a later wave regenerates the ladder (persist unlockedForShopIngredientIds, #206 forward-preserve)",
                                "status": "PROPOSED"},
            },
            "ladder": ladder,
        ...
        "economySafetyRecord": {
            "w1": {"profiles": 4, "reachable": "25/25 for every profile", "deadlock": 0, "unreachable": 0,
                   "circularPrerequisite": 0, "status": "CONFIRMED"},
            "scale101": {"allProfilesComplete": True, "status": "CONFIRMED"},
            "w2PlusEconomyTuningItem": {
                "id": "W2+ ECONOMY TUNING ITEM",
                "finding": "101-target stress: the ★1-equivalent profile reaches a max currency replay streak of "
                           f"{stress['results'][RECOMMENDED]['simulations']['low']['maxGrindStreak']} Margherita bakes",
                "levers": ["bundle discount for multi-item steps", "T4 price cap", "Lunch Rush income"],
                "w1Blocker": False, "status": "CONFIRMED"},
        },
        "dependencies": {
            "productionVisualP1": VISUAL_P1,
            "remaining": [
                "#205 rework onto the Discovery Ladder (star formula no longer needed for material unlock)",
                "#206 forward-preserve of unknown ingredient ids + unlockedForShopIngredientIds (sanitizeInventory drops unknown ids on main)",
                "#221 REC-01..03 content sign-off, RT-01 reference ring capacity (Parmigiana/Portuguesa/Puttanesca)",
                "#218/#222 Completion Gate partial quantity (REC-04 values stay valid as a lower bound)",
                "#217 OD216-3 capability unlock mode (not needed for W1)",
                "piece/portion migration authority (not decided on main; REC-04 adds no schema change)",
            ],
        },
        "ownerDecisions": [
            {"id": "OD-REC04-1", "topic": "Material unlock", "selected": "DISCOVERY_LADDER",
             "summary": "1 new discovery = next ordered progression step; stars are not a material unlock condition",
             "rejected": ["STAR_LADDER", "AUTHORITY_ABSOLUTE_GATES"], "source": OWNER_DECISION_SOURCE,
             "status": "CONFIRMED_OWNER_DECISION"},
            {"id": "OD-REC04-2", "topic": "First stock", "selected": "ZERO_STOCK_PAID_FIRST_PACK",
             "summary": "stock 0 at unlock, no free starter stock; NEW MATERIAL -> Shop -> first pack -> Free Cooking",
             "rejected": ["ONE_PIZZA_TRIAL"], "source": OWNER_DECISION_SOURCE, "status": "CONFIRMED_OWNER_DECISION"},
            {"id": "OD-REC04-3", "topic": "Pack / price", "selected": "PACK10_GENTLE_60_80_100_120",
             "summary": "pack = 10 x k (10 pizza plays); tiers 60/80/100/120; refill 30/40/50/60",
             "rejected": ["PACK10_AUTH_60_100_140_180", "PACK5_GENTLE"], "source": OWNER_DECISION_SOURCE,
             "status": "CONFIRMED_OWNER_DECISION"},
        ],
    }
    return doc
...
def md(doc):
    ...
    L += ["", "## W1 ingredient matrix — DERIVED_FROM_CONFIRMED_RULES", "",
    ...
    L += ["", f"## REC-04 readiness — {doc['verdict']}", "", "| check | pass |", "|---|---|"]
    for k, v in doc["readiness"]["checks"].items():
        L.append(f"| {k} | {v} |")
...
def validate(doc, problems):
    ...
    for d in doc["ownerDecisions"]:
        if d["status"] != "CONFIRMED_OWNER_DECISION":
            problems.append(f"owner decision not recorded as confirmed: {d['id']}")
    cand = CANDIDATES[RECOMMENDED]
    actual = {"gateKind": cand["gate"]["kind"], "unlockFeeFactor": cand["unlockFeeFactor"],
              "firstPackPaid": cand["firstPackPaid"], "trialPizzas": cand["trialPizzas"],
              "grantPizzas": cand["grantPizzas"], "packPizzas": PACK_PIZZAS["recommended"],
              "priceSchedule": PRICE_SCHEDULES[PRICE_SCHEDULE],
              "refillPrices": [math.ceil(x * REFILL_PRICE_FACTOR) for x in PRICE_SCHEDULES[PRICE_SCHEDULE]]}
    if actual != APPROVED:
        problems.append(f"simulated config differs from the approved owner decisions: {actual}")
    for r in doc["recommended"]["w1IngredientMatrix"]:
        if r["packPieces"] != 10 * r["k"] or r["firstStockOnUnlock"] != 0 or r["unlockCost"] != 0:
            problems.append(f"W1 matrix row violates approved pack/first-stock/fee: {r['ingredient']}")
    if not doc["readiness"]["allPass"]:
        failed = [k for k, v in doc["readiness"]["checks"].items() if not v]
        problems.append(f"readiness failed: {failed}")
...
def main(argv=None):
    ...
    if args.check:
        ...
        print(f"PASS REC-04 check: verdict={doc['verdict']}, pins ok, deterministic, recommended findings=0, negative controls "
