...
def overlap(candidate: set[str], production: dict[str, set[str]]) -> dict:
    scored = []
...
def classify(row: dict, production_match: list[str], new_count: int) -> tuple[str, str]:
    if production_match:
...
def build() -> tuple[dict, str, dict, str]:
    ...
    for row in matrix["rows"]:
        ingredients = set(row["ingredients"]["canonicalIngredientIds"])
        new_ids = sorted(ingredients - production_ingredients)
        production_match = mapped_production_ids(row, production_ids)
        ov = overlap(ingredients, production)
        risk, risk_reasons = collision_risk(row, ov)
        wave, rationale = classify(row, production_match, len(new_ids))
        ...
        rows.append({
            ...
            "canonicalIngredientIds": sorted(ingredients),
            "existingIngredientIds": sorted(ingredients & production_ingredients),
    ...
    wave_meta = {
        ...
        "W4": ("Non-structural mechanic queue", "PARTIAL under current flow."),
    ...
    for wave_id in wave_meta:
        if wave_id == "W0":
            ...
            mechanics = sorted(set().union(*(set(row["requiredCapabilities"]) for row in selected))) if selected else []
            ...
            files = sorted(set(FUTURE_FILES["recipe"] + FUTURE_FILES["discovery"] + (FUTURE_FILES["ingredient"] if new_ids else []) + (FUTURE_FILES["sauce"] if any(i.endswith("sauce") or i in {"olive-oil", "pesto"} for i in new_ids) else [])))
    ...
    unresolved = {
        ...
        "rows": [{key: row[key] for key in ("evidenceId", "nameJa", "wave", "currentFlowRepresentability", "productDecisionStatus", "blockers", "reviewItems")} for row in unresolved_rows],
    ...
    writer = csv.DictWriter(csv_buffer, fieldnames=[
        ...
        "representability", "decision_status", "implementation_type", "ingredient_count",
        ...
        "evidence_complete", "blocker_types", "review_item_types", "dedicated_asset_required",
    ...
    for row in rows:
        writer.writerow({
            ...
            "ingredient_count": len(row["canonicalIngredientIds"]), "existing_ingredient_count": len(row["existingIngredientIds"]),
            ...
            "candidate_capabilities": "|".join(row["candidateCapabilities"]), "evidence_complete": row["evidenceComplete"],
