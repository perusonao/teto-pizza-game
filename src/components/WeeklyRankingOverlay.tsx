 ...
 * inside the same `.mission-overlay__panel` shell Lunch Rush's Intro/Result screens already use,
 * so a ranking-read failure of any kind never looks like a different, broken part of the app.
 * Crucially, nothing here can fail Lunch Rush itself: this overlay is only ever reachable from
 * the RESULT screen once a run has already finished, and every one of `getWeeklyLeaderboard`'s
 * outcomes (including a thrown network error) resolves to a typed, renderable state instead of
 * propagating.
 */
export function WeeklyRankingOverlay({ onClose }: WeeklyRankingOverlayProps) {
  ...
  return (
    <div className="mission-overlay">
      <div className="mission-overlay__panel ranking-overlay__panel">
        <button type="button" className="mission-overlay__close" onClick={onClose}>
          閉じる
        </button>
        <h2 className="mission-overlay__title">{"\u{1F3C6}"} 週間ランキング</h2>
