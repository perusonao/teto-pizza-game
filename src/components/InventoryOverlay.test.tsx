...
describe("InventoryOverlay (read-only stock view)", () => {
  ...
  it("test 13: shows a live owned/total summary count", () => {
    ...
    expect(screen.getByText(/所持 3\/14種/)).toBeInTheDocument();
  });
  ...
  it("test 16 (current ~14 ingredients): renders every real, owned ingredient across every category without crashing", () => {
    ...
    expect(screen.getByText(/所持 14\/14種/)).toBeInTheDocument();
