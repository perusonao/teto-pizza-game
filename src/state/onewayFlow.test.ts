...
/**
 * Issue #32 Phase 2: focused reducer-level coverage for the SAUCE -> CHEESE -> TOPPING making
 * flow contract (docs/reports/TETO_ISSUE-32_ONE-WAY-FLOW_Fresh-Audit.md's "Focused test plan").
 * Every assertion here goes through `gameReducer` directly -- no UI, no component harness --
 * because the contract this file exists to pin is "the reducer enforces it, not UI hiding".
 */
...
describe.each([
  ["FREE", false],
  ["Lunch Rush", true],
])("Issue #32 one-way making flow (%s)", (_label, isMissionRound) => {
  describe("SAUCE", () => {
    it("starts a fresh PREPARE round at SAUCE", () => {
  ...
  describe("CHEESE", () => {
    ...
    it("CONFIRM_MAKING_STEP advances CHEESE -> TOPPING", () => {
      const atTopping = confirm(confirm(preparedState(isMissionRound)));
  ...
  describe("ORDER (one-way contract)", () => {
    ...
    it("CONFIRM_MAKING_STEP is a no-op past TOPPING (clamped, never wraps back to SAUCE)", () => {
      const atTopping = confirm(confirm(preparedState(isMissionRound)));
    ...
    it("direct hand-built dispatch sequence cannot bypass the contract: confirm SAUCE, confirm CHEESE, then dispatch COMMIT_SAUCE_DISPENSE is rejected purely by the reducer", () => {
  ...
  describe("RESET (whole-pizza discard/restart)", () => {
    const steps: MakingStep[] = ["SAUCE", "CHEESE", "TOPPING"];

    it.each(steps)("RESET_PIZZA from makingStep=%s deterministically returns to SAUCE", (targetStep) => {
      let state = preparedState(isMissionRound);
      while (state.makingStep !== targetStep) {
        state = confirm(state);
      }
      const reset = gameReducer(state, { type: "RESET_PIZZA" });
      expect(reset.makingStep).toBe("SAUCE");
    ...
    it("a fresh sauce application after RESET_PIZZA works normally (not a stale pointer)", () => {
      ...
      const withSauce = applySauce(reset);
      expect(withSauce.pizza.sauceIds).toEqual([SAUCE_ID]);
    });
