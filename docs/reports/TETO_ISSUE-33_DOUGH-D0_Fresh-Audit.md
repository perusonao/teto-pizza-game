...
- `src/state/gameReducer.ts` — `MakingStep = "DOUGH" | "SAUCE" | "CHEESE" | "TOPPING"`;
  ...
  validity guards mirroring `COMMIT_SAUCE_DISPENSE`).
...
- `src/screens/GameScreen.tsx` — don't render `<IngredientTray>` while `makingStep === "DOUGH"`;
  extend `.prepare-bake-bar`'s CTA branch to a 3-way switch (DOUGH → 次へ, disabled until
  complete; SAUCE/CHEESE → 次へ, unchanged; TOPPING → 焼く！, unchanged).
- `src/data/hints.ts` — one new DOUGH-step hint branch (before/after threshold copy).
- `src/App.css` — new `.pizza-dough-shape`/path styles only; **no changes** to `.pizza-dough`/
  `.pizza-stage` sizing rules.
