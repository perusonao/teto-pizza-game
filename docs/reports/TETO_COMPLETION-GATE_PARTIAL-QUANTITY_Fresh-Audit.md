...
5. **Lunch Rush は 1 本の式では守れない（重要な発見）。**
   - Ranking は `served × 100 + Σquality` なので、1 枚の価値の大部分は「提供した」こと自体にある。
   - 量の係数 Q があっても、少なく載せて早く出す方が、今のモデル（1 個 3 秒）では 45 中 39 で得になる。
   - 係数でこれを防ぐには、不足の係数が 1 個 3 秒で 0.91 以上、4 秒以上では「品質 0 でも得」になる（§7.3）。
   - **推奨: LR-A「Lunch Rush は注文どおりの数を求める」（今の gate のまま）。**
     - ranking の ruleset の bump、Cloud Function の変更、#213 の seam の値の変更がすべて不要になる。
...
### 7.3 1 枚の品質の式だけでは守れない

pizza 1 枚の価値は `100 + quality` で、その過半は「提供した」ことにある。
少なく載せると 1 枚が速くなり、提供の回数が増える。この得を品質の減点だけで打ち消すのに必要な不足の係数（matrix `lunchRushBreakEvenShortCoefficient`。制約になるケースはすべて quattro-formaggi のすべて −1）は次のとおり。

| 1 個あたりの秒数（仮定） | 必要な不足の係数 |
|---|---|
| 1.5 秒 | 0.60 |
| 2 秒 | 0.73 |
| 3 秒 | 0.91 |
| 4 秒 | **1.04（品質 0 でも少なく載せる方が得）** |

→ 実機で 1 個に 2 秒以上かかるなら、recipe mode に合わせた係数（0.5）では Lunch Rush を守れない。係数を上げると、recipe mode の「理想 3 に 2 個 = ★4」が崩れる。**Lunch Rush には mode ごとの判断が要る。**
...
| LR-D | gate は G1 で統一。得になることを受け入れる（Lunch Rush は速さのゲーム） | v2 に bump（適用が変わるので） | 変えない | 1 に変える | ranking の上位が「1 個ずつ」になる。**推奨しない** |
...
| **OD-4** | Lunch Rush の扱い | LR-A: 注文どおりの数が要る（今の gate） / LR-C: gate を統一して提供の点を量で割り引く（ranking v2 と function の deploy） / LR-D: 統一して得になることを受け入れる | **LR-A** | §7。品質の式だけでは速さの得を消せない（1 個 3 秒で係数 0.91 が要る）。LR-A は ranking、function、#213 の seam を変えない |
