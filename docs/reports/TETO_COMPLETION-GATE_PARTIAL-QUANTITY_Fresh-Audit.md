...
5. **Lunch Rush は、recipe mode 用の係数（0.5）では守れない。1 本の係数で両方を満たせるかは、1 個にかかる時間（未実測）次第（Rev.2 で訂正）。**
   - Ranking は `served × 100 + Σquality` なので、1 枚の価値の大部分は「提供した」こと自体にある。
   - 量の係数 Q（0.5）があっても、少なく載せて早く出す方が、今のモデル（1 個 3 秒）では 45 中 39 で得になる。
   - これを防ぐのに必要な不足の係数は、1 個 1.5 秒で 0.60、2 秒で 0.73、3 秒で 0.91、4 秒で 1.04（§7.3）。
     - この係数は**不足率に掛ける**値。制約になるケースは quattro-formaggi のすべて −1（不足率 0.5）で、4 秒でも品質の 48% が残る。
     - **品質 0 の少ない pizza は、1.5〜6 秒のどの仮定でも理想に 1 件も勝たない（45 中 0 件）。** Rev.1 の「4 秒以上では品質 0 でも得」は誤り（§7.3）。
   - recipe mode で「理想 3 に 2 個 = ★4」を保てる係数の上限は **0.711**（pizza-bianca の rosemary 2/3 が制約）。
   - したがって 1 本の係数で両方を満たせるのは、**1 個あたり約 1.94 秒未満のとき**だけ。実機の時間はまだ測っていない。
   - **推奨: LR-A「Lunch Rush は注文どおりの数を求める」（今の gate のまま）を維持する。ただし推奨の強さは下げる。**
     - 1 個の時間に左右されない利点がある: ranking の ruleset の bump、Cloud Function の変更、#213 の seam の値の変更がすべて不要。
     - 実測で 1 個が約 1.9 秒未満なら、「gate を統一して係数を約 0.71 にする」案（LR-E）も成り立つ。
...
### 7.2 G1 を Lunch Rush にもそのまま使うと

matrix `wholePizzaStrategies` の Lunch Rush モデル（**モデルであり実測ではない**。1 枚 = 25 秒 + 3 秒 × (駒の数 + sauce の数)。`efficiency.ts` の authored の定数を唯一の時間の代わりに使った）:

| Model | 少なく載せる方が得（1 run の点数、45 ケース） | 例: funghi 理想 → すべて 1 個 |
|---|---|---|
| 今（G0） | 0 | 835 → 0（FAILED） |
| G1 + S0 | **45** | 835 → 1046 |
| G1 + Q（0.15） | **39** | 835 → 874 |
| G1 + S4 | **41** | 835 → 847 |

### 7.3 品質の式だけで守るのに必要な係数（Rev.2 で訂正）

pizza 1 枚の価値は `100 + quality` で、その過半は「提供した」ことにある。
少なく載せると 1 枚が速くなり、提供の回数が増える。

この得を品質の減点だけで打ち消すのに必要な不足の係数 `a` を求めた（`total × (1 − a × 不足率)`。matrix `lunchRushBreakEvenShortCoefficient`）。

- 制約になるケースは、どの仮定でも quattro-formaggi のすべて −1。4 つの具材が 2 → 1 個になり、不足率は 0.5。
- `a` は**不足率に掛ける**値なので、そのケースに残る品質の割合は `1 − a × 0.5`。
- 「品質 0 の少ない pizza」は別に計算した: `100 / 少ない pizza の秒数` と、理想の `(100 + 99.5) / 理想の秒数` の比較。

| 1 個あたりの秒数（仮定） | 必要な係数 `a` | 制約になるケースの損益分岐の品質 | 残る品質の割合 `1 − a × 0.5` | 品質 0 の少ない pizza（点/秒） | 理想（点/秒） | 品質 0 で理想に勝つケース |
|---|---|---|---|---|---|---|
| 1.5 秒 | 0.60 | 68.4 | 0.70 | 3.08 | 5.18 | 0 / 45 |
| 2 秒 | 0.73 | 62.4 | 0.64 | 2.86 | 4.64 | 0 / 45 |
| 2.5 秒 | 0.83 | 57.5 | 0.59 | 2.67 | 4.20 | 0 / 45 |
| 3 秒 | 0.91 | 53.5 | 0.55 | 2.50 | 3.84 | 0 / 45 |
| 4 秒 | 1.04 | 47.2 | 0.48 | 2.22 | 3.27 | 0 / 45 |
| 6 秒 | 1.21 | 38.9 | 0.40 | 1.82 | 2.53 | 0 / 45 |

> **訂正（Codex review P2）:** Rev.1 は 4 秒の行を「1.04（品質 0 でも少なく載せる方が得）」としていた。これは誤り。
> - 1.04 は不足率 0.5 に掛けるので、品質の約 48% が残る。
> - 実際に品質 0 の少ない pizza は 2.22 点/秒で、理想の 3.27 点/秒に負ける。
> - 品質 0 の少ない pizza が理想に勝つケースは、1.5〜6 秒のどの仮定でも 0 件。品質 0 の少ない pizza の点/秒は、理想の最大 0.78 倍（6 秒のとき）。
> - `a > 1` が意味するのは「不足率 1/a を超える pizza を 0 点まで下げる強さが要る」ということで、品質 0 でも得ということではない。

**recipe mode との両立**（matrix `lunchRushBreakEvenMeta`）:

- recipe mode の設計意図「残り 2/3 以上の不足（理想 3 に 2 個、4 に 3 個）は ★4」を、最良の配置で保てる `a` の上限は **0.711**（制約は pizza-bianca の rosemary 2/3）。
- 必要な `a` がこの上限を下回るのは、**1 個あたり約 1.94 秒未満**のときだけ（二分法で求めた）。
- 1.94 秒未満なら、gate を統一して係数を約 0.71 にすれば両方を満たせる。ただし 2/3 の pizza は ★4 の下限ぎりぎりになり（配置が少しずれると ★3）、Q（0.5）より厳しい。
- 1.94 秒以上なら、1 本の係数では両立しない。係数を上げると recipe mode の「理想 3 に 2 個 = ★4」が崩れる。

→ **Lunch Rush を recipe mode と同じ式で守れるかは、実機の 1 個あたりの時間で決まる。この時間はまだ測っていない**（モデルの 3 秒は `efficiency.ts` の authored の定数で、実測ではない）。
Rev.1 の「1 本の式では守れない」は言い過ぎだったので、この条件つきの記述に改めた。
...
| LR-D | gate は G1 で統一。得になることを受け入れる（Lunch Rush は速さのゲーム） | v2 に bump（適用が変わるので） | 変えない | 1 に変える | 係数 0.5 では、ranking の上位が「少なく載せる」pizza になる。**推奨しない** |
| LR-E（Rev.2 で追加） | gate は G1 で統一。不足の係数を約 0.71 に上げて、recipe mode と Lunch Rush で同じ式にする | v2 に bump | 変えない | 1 に変える | 一貫している。**1 個あたり約 1.94 秒未満の実測が前提**。それ以上だと Lunch Rush で少なく載せる方が得になる。recipe mode の 2/3 は ★4 の下限ぎりぎり |

LR-C の数値の例（quattro-formaggi のすべて −1）: 提供の点 50 + quality 73.5 = 123.5、40 秒。1 秒あたり 3.09 で、理想の 3.84 を下回る。LR-C なら守れる。ただし ranking と function の変更が要り、#215 の範囲を広げる。

**LR-A の推奨の再評価（Rev.2）:**

- Rev.1 は「品質 0 でも得」という誤った数値を LR-A の根拠の 1 つにしていた。訂正後の正しい根拠は次のとおり。
  1. recipe mode 用の係数 0.5 では、1 個 3 秒のモデルで、少なく載せる方が 45 中 39 で得になる（§7.2。変わらない）。
  2. 1 本の係数で両方を満たせるのは、1 個あたり約 1.94 秒未満のときだけ（§7.3）。そのときも、recipe mode の 2/3 は ★4 の下限ぎりぎりになる。
  3. LR-A は、1 個の時間がどうであっても、ranking の ruleset、Cloud Function、#213 の seam を変えずに済む。
- **判定: LR-A の推奨は維持する。ただし強さを下げる**（「1 本の式では守れないので必須」から、「時間が未実測なので、変更が最も少なく安全な既定値」へ）。
- 実装の前に、Human Replay（HR-8）で実機の 1 個あたりの時間を測る。約 1.9 秒を明らかに下回るなら、LR-E（統一、係数約 0.71）を owner にもう一度提示する。
...
| HR-8（Rev.2 で追加。実装の前に測る） | 390×844 の実機で、具材 1 個をトレイから pizza にのせる時間を測る（例: funghi と quattro-formaggi を理想量で各 3 回。TOPPING の step の `perStepElapsedMs` ÷ 駒の数） | 1 個あたりの秒数 | §7.3 の境界（約 1.94 秒）の上か下か。OD-4 の LR-A と LR-E の選択の材料 |
...
| **OD-4** | Lunch Rush の扱い | LR-A: 注文どおりの数が要る（今の gate） / LR-C: gate を統一して提供の点を量で割り引く（ranking v2 と function の deploy） / LR-D: 統一して得になることを受け入れる / LR-E: 統一して不足の係数を約 0.71 に上げる（Rev.2 で追加） | **LR-A（強さは Rev.2 で下げた）** | §7.3、§7.4。係数 0.5 では、1 個 3 秒のモデルで少なく載せる方が 45 中 39 で得。1 本の係数で両立するのは 1 個あたり約 1.94 秒未満のときだけ（実機の時間は未実測）。品質 0 の少ない pizza は、どの仮定でも理想に勝たない。LR-A は時間に関係なく、ranking、function、#213 の seam を変えない。HR-8 の実測で 1.9 秒を明らかに下回るなら、LR-E を再提示する |
