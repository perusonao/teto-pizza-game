...
- sauce-spread animation の originはタップ位置を `.pizza-dough`（全体）基準の
  %で計算しているが、`clip-path` の `at X% Y%` は `.pizza-sauce-layer`
  （`inset: 6%` 分小さい）基準で解決されるため、ごく僅かな位置のズレがある
  （最大で見た目上数%程度）。実プレイでは知覚できないレベルだが、座標系を
  完全に統一する余地はある（Phase 2Bの9章で記録済みの、タップ座標とCSS配置
  座標のスケール差と同種の既知の限界）
