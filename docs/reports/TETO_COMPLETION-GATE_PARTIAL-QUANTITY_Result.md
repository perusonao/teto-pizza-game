...
Audited main **`a8e7888`**（#206 I0 save forward compat、#225 IngredientGlyph、#226 RT-01a/b の merge 後）。

- 1 回目: PR head `81a850c` に main `46513b1` を merge commit `23d9220` で取り込み、CI（WebKit Gate を含む）がすべて green（HEAD `b2549e8`、run 36109740959）。
- その後 main が `a8e7888` に進んだので、main をもう一度 merge commit で取り込んで gate をやり直した（rebase・force-push なし）。
- `46513b1..a8e7888` と PR の変更ファイルは `src/App.css` と `src/components/ResultPanel.tsx` の 2 つが重なるが、hunk は別の場所で auto-merge（textual conflict 0 件）。`ResultPanel.tsx` の main 側は original pizza の具材一覧を `IngredientGlyph` にしただけで、この PR の数量の 1 行とは独立。
- RT-01 は player reference の配置（`playerReference.ts` / `pizzaReferenceLayout.ts`）と thumbnail だけを変える。Scoring 2.0 が読む `referencePizza.ts`、`scoringV2/**`、`completionGate.ts`、`recipes.ts` は main 側で変更なし。そのため quantity factor の target（Reference の `positions.length` = `minCount`）は変わらない。

| # | Check | Result |
|---|---|---|
| 1 | Owner Decision | #215 の OD-1 G1 / OD-2 0.5 / OD-3 0.15 / OD-4 LR-A / OD-4b 0.15 / OD-5 D-A / OD-LR-RANKING（案 A）と実装が一致。新しい仕様判断なし |
| 2 | Semantic conflict | main 側の変更は save（`persistence.ts`）、WebKit CI script、具材の glyph（#225）、player reference の配置と thumbnail（#226）。この PR はどれにも触れず、Scoring 2.0 / Completion Gate の入力も main 側で変わらない（上の箇条書き）。`ScoringV2Result`・`completion` は `GameState` の一部で、save には書かれない |
...
| 6 | 在庫の負数禁止 | `consumePizzaInventory` は置いた個数だけ `Math.max(0, …)` で減らす（変更なし。main 側でも変更なし）。1 回目の merge 後の tree（`23d9220`）で scratch test（commit しない）: funghi mushroom 2/3、在庫 2 / 1 / 0 → PASS、在庫は 0 / 0 / 0、負数なし、CONFIRM_BAKE を繰り返しても二重消費なし |
...
検証（main `a8e7888` を merge した後の tree）:
