...
- Codex P1 blocker fix Round 2 (strict fail-closed Reference validation) HEAD SHA: see the commit this report ships in (third commit on this branch) — see "Codex P1 Blocker Fix Round 2" section below.
...
**Player-input vs. authoritative-Reference distinction.** Deliberately NOT changed: `scorePieceGroupV2` (the low-level per-group scorer) and every player-input sanitizer (`sanitizeToppings`, `sanitizeSauceDeposits`, `sanitizeStringArray`, `sanitizeCoordinates`) keep their Round 1 lenient, drop-and-continue behavior. `scorePieceGroupV2` is now explicitly documented as "the crash/hang-safety layer for player input; Reference strictness is enforced one level up in `scorePiecesComponentV2`" — direct callers (including its own pre-existing Round 1 tests) still get filtered/lenient scoring against a possibly-malformed `group`, which is correct for that layer's contract. Only the two truly authoritative collections (`recipe.requiredIngredients`, `ReferencePieceGroup.positions`/`.matching`/the `groups` container) go through the new strict gate.
...
- After the Codex P1 blocker fix, Round 2 (this update): **42 test files, 733 tests, all passing** (+34 tests net in `malformedInput.test.ts`; 3 narrowing-only assertions added to `scoringV2.test.ts`'s pre-existing tests, 0 assertion values weakened anywhere).
...
4. **`computeScoringV2Shadow`'s `pieces`-unavailable propagation branch has no reachable end-to-end test with real data** — since `getReferencePizza`'s static `MARGHERITA_REFERENCE` is always valid, only the `recipe.requiredIngredients` path (a genuine public parameter) could be exercised end-to-end; the `pieces`-unavailable path is fully covered at the `scorePiecesComponentV2` unit level instead, and the propagation code itself is straightforward/symmetric with the `recipeComponent` branch it sits beside. Defense-in-depth for a future (possibly malformed) additional Reference fixture, not a gap in current behavior.
