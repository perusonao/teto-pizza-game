...
- Codex P1 blocker fix (Round 1) HEAD SHA: `093c5d6d3d385f3d6b6b44d2a2223b4fc225854b` — see "Codex P1 Blocker Fix" section below.
- Codex P1 blocker fix Round 2 (strict fail-closed Reference validation) HEAD SHA: `87ad55da0da7b0bf8b4672b61750577b0502bff4` — see "Codex P1 Blocker Fix Round 2" section below.
- Codex P1 blocker fix Round 3 (close the `scorePieceGroupV2` direct-call bypass) HEAD SHA: see the commit this report ships in (fourth commit on this branch) — see "Codex P1 Blocker Fix Round 3" section below.
...
## Codex P1 Blocker Fix Round 2

**Remaining failure** (Codex narrow-verification, PR #31, reviewed HEAD `093c5d6d3d385f3d6b6b44d2a2223b4fc225854b`, verdict "B. BLOCKER REMAINS — FIX REQUIRED"): crash/hang safety from Round 1 held, but strict fail-closed semantics were still incomplete. Codex verified that malformed *authoritative* Reference/requirement containers, and a Reference position list with even a single malformed position mixed into otherwise-valid ones, could still produce a valid Shadow score — including 100. No `docs/reports/TETO_PR-31_SCORING-2_Shadow_Codex-Review.md` was present in the working tree, so the failure was independently reproduced from the blocker description before any fix was written.

**Why filtering malformed authoritative requirements was unsafe.** Round 1's sanitizers (`./boundary.ts`'s `sanitize*` functions) are correct for PLAYER input: a malformed topping/deposit genuinely means "the player didn't place a real piece here", so dropping it and scoring the rest is the right, tolerant reading. But Round 1 applied the *same* drop-and-continue logic to `recipe.requiredIngredients`, `ReferencePieceGroup.positions`, and the `pieceGroups`/`groups` container itself — data that isn't player input at all, it *is* the scoring truth a player's pizza is judged against. Concretely reproduced:

- `recipeComponent.ts`: a non-array `recipe.requiredIngredients` (or one with every element malformed) filtered down to a zero-length list, and `required.length === 0` was read as "this recipe legitimately requires nothing" — awarding a full, undeserved 100.
- `piecesComponent.ts`: a `group.positions` list with even one malformed position (e.g. `[validPos, NaN-pos, null]`) filtered down to just the valid position(s) — shrinking the *target* itself. A player who placed exactly one piece at that one surviving valid position then scored a near-perfect match against a corrupted, artificially-easy target, even though the real Reference required three. The same failure existed at the `groups` container level: a non-array `pieceGroups` filtered down to `[]` and `scoredGroups.length === 0` read as "no piece groups required at all" — again a full, undeserved 100.

Both are the same root mistake: treating "the authoritative data got smaller after filtering" as a fact about the recipe, when it is really a symptom of the data being corrupt.

**Strict Reference validation design.** New `./boundary.ts` exports (additive, no existing sanitizer changed): `StrictReferenceValidation<T>`, `validateCoordinateArrayStrict`, `validateRequiredIngredientsStrict`, and the shared `MALFORMED_REFERENCE_REASON` message. Unlike the lenient sanitizers, these are all-or-nothing — a non-array container, or *any single* malformed element anywhere in the array, invalidates the whole collection (`{ valid: false }`), never a `{ valid: true, items: <filtered subset> }`. A genuinely well-formed empty array is still `{ valid: true, items: [] }`, so a recipe that legitimately has no requirements (or a hypothetical Reference with no piece groups) is unaffected. Wired in at exactly the two places Codex named:

- `recipeComponent.ts`'s `scoreRecipeComponentV2` now validates `recipe.requiredIngredients` with `validateRequiredIngredientsStrict` before anything else; an invalid result returns `{ available: false, reason: MALFORMED_REFERENCE_REASON }` instead of a `RecipeComponentV2`. Its return type widened to `RecipeComponentV2 | ScoringV2Unavailable` (a union `types.ts` already declared for `ScoringV2Components.recipe`, just never previously produced).
- `piecesComponent.ts` gains a new `isGroupReferenceDataValid` gate, called from `scorePiecesComponentV2` for every group *before* any scoring: it strictly validates `group.ingredientId` (must be a string), `group.positions` (via `validateCoordinateArrayStrict` — one bad position anywhere fails the whole group), and `group.matching`'s tolerance radii (both must be numbers forming a valid band). A non-array `groups` container, or any single invalid group within it, returns `{ available: false, reason: MALFORMED_REFERENCE_REASON }` for the whole `PiecesComponentV2` before `scorePieceGroupV2` is ever called. Return type widened to `PiecesComponentV2 | ScoringV2Unavailable`.
- `index.ts`'s `computeScoringV2Shadow` now checks `pieces.available`/`recipeComponent.available` after computing them; if either is `false`, the *whole* `ScoringV2Result` fails closed (`available: false, totalScore: null`, reusing the exact P0-1 "no Reference fixture at all" shape) rather than combining a broken component's absence into a partial total. `ScoringV2ShadowPanel.tsx` needed no changes — it already only reads `result.components` when `result.available` is true.

**Player-input vs. authoritative-Reference distinction.** Deliberately NOT changed: every player-input sanitizer (`sanitizeToppings`, `sanitizeSauceDeposits`, `sanitizeStringArray`, `sanitizeCoordinates`) keeps its Round 1 lenient, drop-and-continue behavior. Round 2 left `scorePieceGroupV2` (the low-level per-group scorer) itself lenient too, with the strict gate living only in its caller `scorePiecesComponentV2` — **this specific design decision is superseded by Round 3 below**, which found and closed a direct-call bypass around it. Only the two truly authoritative collections (`recipe.requiredIngredients`, `ReferencePieceGroup.positions`/`.matching`/the `groups` container) go through the strict gate; player input is unaffected by either round.

**New regression cases** (`malformedInput.test.ts`, extensively revised): the two Round 1 `describe` blocks that had asserted the *vulnerable* behavior as correct (`"recipe.requiredIngredients is %s -> treated as no requirements, score 100"` and `"groups container is %s -> full marks"`) were inverted to assert `available: false` instead — these were literally encoding the bug Codex found. Added: every malformed-container shape (null/undefined/object/string/number/boolean) against both `recipe.requiredIngredients` and the `groups` container; one malformed requirement mixed with valid ones; a malformed `minCount` (missing/wrong type/NaN); a full malformed-position matrix (missing x, missing y, wrong x/y type, NaN, +Infinity, -Infinity, null element, primitive element) applied to a single position mixed into an otherwise-valid mozzarella list and, separately, an otherwise-valid basil list; the exact Round-1 "corrupted-down target trivially completable for >90" exploit shape reproduced and reconfirmed now fails closed; malformed `group.matching` (missing entirely, wrong-typed radii); and an end-to-end `computeScoringV2Shadow` case proving a malformed `recipe.requiredIngredients` fails the *whole* result closed even when Sauce/Pieces would otherwise score well. Every new case also has a paired "fully valid input still scores/is available normally" test, so the stricter gate's absence-of-regression is pinned alongside its presence-of-fix.

**Valid Golden Matrix unchanged evidence.** `scoringV2.test.ts`'s full pre-existing Golden Matrix suite (perfect/good/poor/empty ordering, all sauce variants) passes unmodified except for three `assertAvailable()` narrowing calls added purely to satisfy the now-widened return types — no assertion values changed. New "fully valid ... still scores normally" tests in both the strict-Pieces and strict-Recipe describe blocks reconfirm `result.score > 90` for the real Margherita Reference data, and a new end-to-end test confirms a fully valid recipe + a well-made pizza still scores `available: true, totalScore > 80`.

**Exact tests.** `malformedInput.test.ts` grew from 136 to 170 tests (Round 1's 136, minus the 2 inverted-assertion tests that were rewritten, plus 36 new Round 2 cases). Combined suite total below.

## Codex P1 Blocker Fix Round 3

**Exact remaining bypass** (Codex narrow-verification, PR #31, reviewed HEAD `87ad55da0da7b0bf8b4672b61750577b0502bff4`, verdict "B. BLOCKER REMAINS — FIX REQUIRED"): the aggregate/component path (`scorePiecesComponentV2` → `computeScoringV2Shadow`) now failed closed correctly, but `scorePieceGroupV2` itself — exported, and independently callable — still sanitized/filtered malformed authoritative Reference positions before Hungarian matching. Codex reproduced: a direct `scorePieceGroupV2(toppings, group)` call with `group.positions` containing one valid position mixed with several malformed ones still filtered the malformed ones out, computed `targetCount === 1` (the single surviving valid position), and returned a normal score up to and including 100 for a player who trivially "completed" that shrunk-down, corrupted target. Round 2's own pre-existing test (`"group.positions contains malformed Reference position elements (mixed valid + invalid) -> matches only the valid ones, never hangs, never throws"`, asserting `result.score > 90`) was itself pinning this exact vulnerable behavior as correct.

**Root cause: the strict gate lived one layer too high.** Round 2 put `isGroupReferenceDataValid` (the strict validator) only inside `scorePiecesComponentV2`, running it *before* calling `scorePieceGroupV2` for each group. That correctly protects the path every real caller (`computeScoringV2Shadow`) actually uses — but `scorePieceGroupV2` was still exported as its own public function with no gate of its own, so calling it directly (as a test, or any future caller, could) skipped the check entirely. This is precisely "relying on callers to remember to validate" rather than making the primitive itself safe.

**`scorePieceGroupV2` API decision.** Per the task's preferred-solution guidance ("make `scorePieceGroupV2` itself enforce strict validation... before any filtering or Hungarian matching") and "choose the smallest change consistent with the current architecture": the strict validation logic (previously the standalone `isGroupReferenceDataValid` helper, called only from `scorePiecesComponentV2`) was moved to be the first thing `scorePieceGroupV2` itself does. `scorePieceGroupV2`'s return type widened from `PieceGroupScoreV2` to `PieceGroupScoreV2 | ScoringV2Unavailable` (mirroring every other Scoring 2.0 component function) — `types.ts`'s `PieceGroupScoreV2` gained an `available: true` discriminant to make this a proper union. The function keeps its real parameter types (`PlacedTopping[]`, `ReferencePieceGroup`) for callers' compile-time ergonomics, but treats both as `unknown` internally before any property access, exactly like the rest of this module. `scorePiecesComponentV2`'s own separate pre-check is now redundant and was removed — it simply calls `scorePieceGroupV2` per group and checks each result's own `available` field, propagating `available: false` up if any group's Reference data failed validation. There is now exactly one implementation of the strict-Reference-position-validation rule, and every path — direct call or through the aggregate function — goes through it. `isGroupReferenceDataValid` as a separate named helper no longer exists; its checks are inlined directly in `scorePieceGroupV2` so the same intermediate typed values (the validated `positions` array, the validated `fullCreditRadius`/`zeroCreditRadius`) can be reused for scoring without a second, separately-typed re-validation call.

**Strict Reference validation location.** Unchanged from Round 2: `./boundary.ts`'s `validateCoordinateArrayStrict` (positions) and the same `isValidToleranceBand` (matching radii) this module already used. No new validators were needed — only where the existing ones are called from changed.

**Vulnerable test expectation that was inverted.** `malformedInput.test.ts`'s `"group.positions contains malformed Reference position elements (mixed valid + invalid) -> matches only the valid ones, never hangs, never throws"` (previously asserting `result.targetCount === 1` and `result.score > 90`) was replaced by `"2. mixed valid + invalid Reference positions -> available:false (THE exact bypass Codex reproduced: this used to return targetCount 1 / score 100)"`, asserting `result.available === false` for the exact same input shape. The whole surrounding `describe` block (previously titled "adversarial toppings/group input", documented as testing an intentionally-lenient primitive) was renamed and rewritten as `"scorePieceGroupV2 -- DIRECT-entry adversarial matrix (Codex P1 Round 3)"`.

**Direct-entry adversarial matrix** (`malformedInput.test.ts`, all calling `scorePieceGroupV2` directly, never `scorePiecesComponentV2`): (1) fully valid Reference — normal score, unchanged; (2) mixed valid + invalid Reference positions — the exact reproduced bypass, now `available: false`; (3) null element; (4) primitive element; (5–9) the full `MALFORMED_POSITION_CASES` matrix (missing x, missing y, wrong x/y type, NaN x/y, +Infinity x/y, -Infinity x/y) each mixed into an otherwise-valid position list; a non-array `group.positions` container; the `group` argument itself malformed (null/undefined/object/string/number/boolean, via `MALFORMED_CONTAINERS`); `group.matching` missing entirely; `group.matching` with wrong-typed radii; `group.matching` forming an invalid tolerance band (`zero <= full`) even when both radii are well-typed numbers. Every one of these asserts `available: false` and `not.toThrow()`. Paired with these: the player-side (`toppings`) matrix stays lenient and unchanged — a malformed `toppings` container or malformed elements within it still return `available: true` with the malformed pieces simply not counted, exactly as Round 1/2 established.

**Call-site/export audit.** `grep -rn "scorePieceGroupV2"` across `src/` confirms: it is defined and exported exactly once, in `piecesComponent.ts`; its only production (non-test) caller is that same file's own `scorePiecesComponentV2`; `index.ts` (the module's actual public entry point, the only file `gameReducer.ts`/`App.tsx`/`ScoringV2ShadowPanel.tsx` ever import from) re-exports only `computeScoringV2Shadow`, `SCORING_V2_RULESET_VERSION`, and types — never `scorePieceGroupV2` or `scorePiecesComponentV2` as values. No production file imports `./piecesComponent` directly except `index.ts` itself (confirmed via a second grep for import statements). The only reachable ways to call `scorePieceGroupV2` at all are (a) importing `./piecesComponent` directly, which only test files do, or (b) transitively through `scorePiecesComponentV2` → `computeScoringV2Shadow`. Since the strict gate now lives in `scorePieceGroupV2` itself, both paths are safe identically — there is no remaining second reachable path that bypasses it.

**Valid-input regression evidence.** `scoringV2.test.ts`'s full `scorePieceGroupV2`/`scorePiecesComponentV2` describe block (Reference-exact placement, slightly/badly displaced pieces, missing pieces, extra pieces, permutation invariance, the "no double-penalizing" Recipe-vs-Pieces separation test) still passes with only `assertAvailable()` narrowing calls added — no assertion values changed. The Golden Matrix suite (`perfect > good > poor > empty`) and the permutation-invariance test in `malformedInput.test.ts` both still pass unmodified in substance. FREE/Lunch Rush integration (`gameReducer.test.ts`), legacy authority (score/stars/BEST/rewards), save-v1 isolation (`persistence.test.ts`), and Preview/Production gating (`ScoringV2ShadowPanel.test.tsx`, production `vite build` grep) were all re-run and re-confirmed unchanged.

**Exact tests.** `malformedInput.test.ts` grew from 170 to 191 tests. Combined suite total below.
...
- After the Codex P1 blocker fix, Round 1 (`093c5d6`): **42 test files, 699 tests, all passing**.
- After the Codex P1 blocker fix, Round 2 (`87ad55d`): **42 test files, 733 tests, all passing**.
- After the Codex P1 blocker fix, Round 3 (this update): **42 test files, 754 tests, all passing** (+21 tests net: the `scorePieceGroupV2` direct-entry matrix grew substantially, offset by removing the now-superseded lenient-primitive tests it replaced; 0 assertion values weakened anywhere in the suite).
- `npx tsc -b`: clean, no errors.
- `npx oxlint`: clean, no warnings.
- `npx vite build`: succeeds, production bundle confirmed free of Shadow debug UI (re-confirmed after this fix).
...
2. ~~**Hungarian-matcher NaN/Infinity hang**~~ — **addressed at the Scoring 2.0 boundary by the Codex P1 blocker fixes above**, now enforced at the single point (`scorePieceGroupV2` itself, as of Round 3) every reachable path -- direct or aggregate -- goes through, for both the player-topping side and the Reference-position side. `../referenceMatching.ts`'s `minimumCostColumns` primitive itself is still unfixed at its source — a malformed cost could still hang it if some *other*, non-Scoring-2.0 caller ever fed it unsanitized data directly (currently only `referenceScoring.ts`'s live-preview path calls it, always with reducer-validated `PizzaState` data). Recommend a small, isolated follow-up PR to harden the primitive itself so every caller benefits, not just Scoring 2.0's own boundary.
...
4. **`computeScoringV2Shadow`'s `pieces`-unavailable propagation branch still has no reachable end-to-end test through `computeScoringV2Shadow` itself with real data** — since `getReferencePizza`'s static `MARGHERITA_REFERENCE` is always valid, only the `recipe.requiredIngredients` path (a genuine public parameter of `computeScoringV2Shadow`) could be exercised end-to-end at that top level; the `pieces`-unavailable path itself is now thoroughly covered at both the `scorePieceGroupV2` direct-entry level (Round 3's matrix) and the `scorePiecesComponentV2` level, and the propagation code in `index.ts` is straightforward/symmetric with the `recipeComponent` branch it sits beside. Defense-in-depth for a future (possibly malformed) additional Reference fixture, not a gap in current behavior.
5. **Calibration nuance**: a full-quantity-but-badly-distributed sauce dump can currently outscore a well-distributed-but-under-quantity spread (see Golden Matrix). Both are legitimate distinct mistakes; the 30/30/20/20 sauce sub-weights simply favor correcting quantity over distribution right now.
6. **Lunch Rush → Margherita was not reproducible live in a single browser session** due to Mission's own pre-existing "never repeat the just-active recipe" rule combined with the session always starting on Margherita in FREE mode; covered instead by a deterministic reducer-level integration test, plus a live run against Lunch Rush's Reference-unavailable path (Marinara) exercising the exact same rendering code.
7. **`scorePiecesComponentV2`'s equal 50/50 weighting** across mozzarella/basil groups is provisional — documented in-code as not a claim that every future ingredient type should weigh the same.
