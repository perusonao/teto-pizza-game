...
|---|---|---|---|---|---|
| `EVIDENCE_STRICT` | nothing | 87 (all evidence-ready rows) | **0** | 87/87 | **COMPLETE for every skill, but with no first-bake discovery.** Before the first discovery the player bakes original pizzas, which pay Pitz: 1 for SKILLED and STANDARD, 2 for BEGINNER, 3 for WORST (`originalPizzaBakesBeforeFirstDiscovery`). The step-1 gate is 0⭐, so the player buys eggplant and discovers **Melanzane** at bake 2 (SKILLED/STANDARD), 3 (BEGINNER) or 4 (WORST). `originalPizzaBakes` is the lifetime count, which also includes later original-pizza income; WORST reaches 8. Negative control: if original pizzas paid 0 Pitz, the result would be START_DEADLOCK. |
